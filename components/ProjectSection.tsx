...
import ProjectSectionPayload from '../types/frontPage/ProjectSection.interface';


const colors = ['aaa', 'bbb', 'ccc', 'ddd', 'eee']

const ProjectSection: React.FC<ProjectSectionPayload> = ({
	title,
	featuredText,
	buttonText,
	project
		...
		<div className={styles.container}>
			<AnimatePresence>
				<motion.div
					...
					<div className={`${styles.title} mb-8`}>{title}</div>
			...
			<AnimatePresence>
				<motion.div className={styles.featured}
					...
					<div className="flex flex-col">
						<div className={`${styles.title} text-kapala-shadow mb-8`}>{featuredText}</div>
						<div className="text-kapala-shadow opacity-80 text-2xl font-medium pb-4 border-b-4 border-b-kapala-700/30">{project.title}</div>
						<div className="pt-4 text-kapala-20 font-thin mb-8">{project.desc}</div>
			...
			<Project {...project} visible={visible} setVisible={setVisible} />
		</div>
	)
}
